	...
	.Does(() =>
{
	...
	PatchAssemblyInfo("../src/Opten.Umbraco.ListView/Properties/AssemblyInfo.cs", version);

	FileWriteText(dest + File("Opten.Umbraco.ListView.variables.txt"), "version=" + version);

	Information("Patch files with ?v: {0}", ReplaceTextInFiles(
       "../src/Opten.Umbraco.ListView.Web.UI/App_Plugins/**/*",
       "?v=ASSEMBLY_VERSION",
       "?v=" + version
	).Count());
...
Task("Restore-NuGet-Packages") 
	...
	.Does(() =>
{ 
	NuGetRestore("../Opten.Umbraco.ListView.sln", new NuGetRestoreSettings {
		NoCache = true
	}); 
});
...
Task("Build") 
	...
	.Does(() =>
{
	MSBuild("../src/Opten.Umbraco.ListView/Opten.Umbraco.ListView.csproj", settings =>
...
Task("Pack")
	...
	.Does(() =>
{
	NuGetPackWithDependencies("./Opten.Umbraco.ListView.nuspec", new NuGetPackSettings {
		Version = version,
		BasePath = "../",
...
Task("Deploy")
	.Does(() =>
{
	string packageId = "Opten.Umbraco.ListView";
	...
	Slack(new SlackSettings {
		ProjectName = "Opten.Umbraco.ListView"
	});
});
